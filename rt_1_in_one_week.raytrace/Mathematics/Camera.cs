...
namespace rt_1_in_one_week.raytrace.Mathematics
{
        ...
        : ICamera
    {
        ...
        public static Camera CreateLookAt(Vec3 lookFrom, Vec3 lookAt, Vec3 vup, double vfov, double aspect, double aderpture = 0, double fous_dist=1)
        {
            double theta = vfov * Math.PI / 180;
            double half_height = Math.Tan(theta / 2);
            double half_width = half_height * aspect;
        ...
        public Camera(Vec3 lower_left_corner, Vec3 horizontal, Vec3 vertical, Vec3 origin, double lense_radius)
        {
        ...
        public Ray Get(double u, double v)
        {
            ...
            var ray = new Ray(_origin + offset, _lower_left_corner + _horizontal * u + _vertical * v - _origin - offset);
            return ray;
        }
    }
}
